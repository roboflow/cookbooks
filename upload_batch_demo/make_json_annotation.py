...
for folders in get_folders:
    if "." in folders:
        ...
        for images in get_images:
            ...
            print("*** Processing image [" + str(image_count) + "/" + str(len(get_images)) + "] - " + ".json" + " ***")
            ...
            for objects in jsonLoaded['predictions']:
                ...
                objects["image_path"] = image_name
                
                print(objects["image_path"])

            else:

                print("PREDICTION NOT FOUND AT THRESHOLD")

            if jsonLoaded['predictions']:

                print("JSON FILE WRITTEN")

                jsonFile = open(images[:-4]+'.json', "w")

                # Pretty Print JSON
                json_formatted_str = json.dumps(jsonLoaded, indent=4)
                # print(json_formatted_str)

                jsonFile.write(json_formatted_str)
                jsonFile.close()
