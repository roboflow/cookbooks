...
def create_pipeline(fps, focus):
    ...
    # Create nodes for first camera (CAM_A), imx378
    ...
    # Create nodes for second camera (CAM_D), ov9782
    cam_rgb2 = pipeline.create(dai.node.ColorCamera)
    xout_rgb2 = pipeline.create(dai.node.XLinkOut)
    cam_rgb2.setBoardSocket(dai.CameraBoardSocket.CAM_C)
    cam_rgb2.setResolution(dai.ColorCameraProperties.SensorResolution.THE_800_P)
    cam_rgb2.setFps(fps)
...
def main(name, fps, focus):
    ...
    with dai.Device(pipeline) as device:
        ...
        output_filename2 = f"{output_folder_name}oak_ov9782_fixed_focus_{name}_{timestamp}.mp4"

        fourcc1 = cv2.VideoWriter_fourcc(*'mp4v')
        out1 = cv2.VideoWriter(output_filename1, fourcc1, 30.0, (2160, 3840))

        # FFmpeg command setup for the second camera
        ffmpeg_cmd = ['ffmpeg', '-y', '-f', 'rawvideo', '-vcodec', 'rawvideo',
                      '-s', '600x800', '-pix_fmt', 'rgb24', '-r', str(fps),
        ...
        while True:
            ...
            if in2 is not None:
                ...
                frame2 = cv2.resize(frame2, (600, 800))
                cv2.imshow("Camera 2", frame2)
                try:
                    ffmpeg.stdin.write(frame2.tobytes())
